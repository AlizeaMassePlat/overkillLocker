    ...
    <ion-content>
      ...
      <div class="bookings-container">
        ...
        <div
          ...
          :key="booking.id"
          :class="['booking-item', { 'active-booking': booking.state === 1 }]"
  ...
  setup() {
    ...
    const fetchBookings = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3000/reservation/user/1"
        ); // Replace '1' with actual user_id
        bookings.value = response.data.map((res: any) => ({
          ...res,
          number: `N°${res.id}`, // Generate number from id
          date: new Date(res.date).toLocaleDateString(), // Convert date to string
          image: lockerImg, // Use correct image
        }));
      } catch (error) {
    ...
    const giveBack = async () => {
      ...
      if (selectedBookingId.value !== null) {
        try {
          await axios.patch(
            `http://localhost:3000/reservation/${selectedBookingId.value}`,
            {
              state: 0, // Assuming 0 means returned
            }
          );
          booking.value = { ...booking.value, state: 0 };
          closeBookingDetailModal();
